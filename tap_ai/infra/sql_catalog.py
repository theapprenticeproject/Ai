...
"""
SQL schema catalog loader.

Reads `tap_ai_schema.json` — a static snapshot of the remote PostgreSQL
schema generated by `schema/generate_schema.py` — and caches it in memory
with a 5-minute TTL. All SQL-generation code (sql_answerer, doctype_selector)
calls `load_schema()` to get the current schema rather than hitting the disk
on every request.
